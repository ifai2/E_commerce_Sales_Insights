...
# Page Configuration
# ------------------------------
st.set_page_config(page_title="Seller App", page_icon="💼", layout="centered")

# ------------------------------
# Sidebar Navigation
# ------------------------------
st.sidebar.title("Navigation")
page = st.sidebar.radio("Go to", ["Profitability Predictor", "Seller Analysis"])

# ------------------------------
# Page 1: Profitability Predictor
# ------------------------------
if page == "Profitability Predictor":
...
# ------------------------------
# Page 2: Seller Analysis
# ------------------------------
elif page == "Seller Analysis":
    st.title("Seller Data Analysis")

    st.subheader("Dataset Overview")
    st.dataframe(seller_data.head())

    st.subheader("Summary Statistics")
    st.write(seller_data.describe())

    st.subheader("Distribution of Total Orders")
    fig, ax = plt.subplots()
    ax.hist(seller_data['total_orders'], bins=20, color='skyblue', edgecolor='black')
    ...
    fig, ax = plt.subplots()
    ax.hist(seller_data['total_revenue'], bins=20, color='lightgreen', edgecolor='black')
    ax.set_xlabel('Total Revenue (BRL)')
    ax.set_ylabel('Number of Sellers')
    st.pyplot(fig)

    st.subheader("Average Revenue by City")
    avg_revenue_city = seller_data.groupby('seller_city')['total_revenue'].mean().sort_values(ascending=False).head(10)
    fig, ax = plt.subplots(figsize=(8,4))
    avg_revenue_city.plot(kind='bar', color='orange', ax=ax)
    ax.set_ylabel('Average Revenue (BRL)')
    ax.set_xlabel('City')
    st.pyplot(fig)

    st.subheader("Review Score vs Profitability")
    fig, ax = plt.subplots()
    sns.boxplot(x='profitable', y='avg_review_score', data=seller_data, ax=ax)
    st.pyplot(fig)

    st.subheader("Feature Correlation")
    corr = seller_data[['total_orders', 'total_products_sold', 'avg_price', 'avg_freight',
                        'avg_review_score', 'total_revenue', 'profitable']].corr()
